...
class N_in_line(object):
	...
	def play(self):
		...
		while not self.is_game_ended():
			score, best_move = self.min_max(self.player1, depth=self.n, alpha=-math.inf, beta=math.inf)  # Computing the best move for the bot

			if best_move is not None:
				self.make_move(best_move, self.player1)
				self.represent_board()  # Representing the board
				print(f'The BOT1 moved at position {best_move[0] + 1}, {best_move[1] + 1}\n')
			...
			if best_move is not None:
				...
				self.represent_board()  # Representing the board
				print(f'The BOT2 moved at position {best_move[0] + 1}, {best_move[1] + 1}\n')
