...
def align_profiles(seqs, names_i, names_j, gep):
    ...
    for i in range(1, len_i + 1):
        for j in range(1, len_j + 1):
            ...
            # ABOUT 10 LINES OF CODE; HINT: seq_dict1.values()
            # Iterate over the names and get each sequence to align it.
            for ni in names_i:
                for nj in names_j:
                    if seqs[ni][i - 1] != '-' and seqs[nj][j - 1] != '-':
                        score += subst_mat[seqs[ni][i - 1].upper()][seqs[nj][j - 1].upper()]
                        num_subst += 1
            
            # If there is at least one substitution, divide the score by the number of substitutions.
            if num_subst > 0:
