...
def sub_sum_interval(L, U, t):
	...
	'''
	solutions = []
	def find_subset_sum(index, current_sum, current_subset):
		if L <= current_sum < U:
			solutions.append(current_subset)
		if index == len(t):
			return 
		# Include the current element
		find_subset_sum(index + 1, current_sum + t[index], current_subset + [t[index]])
		# Exclude the current element
		find_subset_sum(index + 1, current_sum, current_subset)
