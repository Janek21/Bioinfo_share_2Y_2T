...
def align_profiles(seq_dict1, seq_dict2, gep):
    ...
    for i in range(1, len_i + 1):
        for j in range(1, len_j + 1):
            ...
            # COPYRIGHT MARTÍ DÍEZ, DO NOT STEAL M****RF*****S

            for value_dict1 in seq_dict1.values():
                for value_dict2 in seq_dict2.values():
                    if value_dict1[i - 1] != '-' and value_dict2[j - 1] != '-':
                        score += subst_mat[value_dict1[i - 1].upper()][value_dict2[j - 1].upper()]
                        num_subst += 1
