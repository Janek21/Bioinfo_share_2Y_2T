...
def move_gaps(seq1, seq2):
    ...
    if len(seq1)>len(seq2):
        gap_words=[seq1] #bigger one in gap_words
        seq=seq2 #smaller one in seq
    else:
        gap_words=[seq2]#
        seq=seq1

    dif=len(gap_words[0])-len(seq)

    for a_pos in range(len(seq)+1):
        gap_words.append(seq[:a_pos]+"-"*dif+seq[a_pos:])
    return(gap_words)
